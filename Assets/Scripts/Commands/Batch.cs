...
namespace DeveloperConsole.Commands
{
    ...
    class Batch : CommandBase
    {
        ...
        public override void Execute(string[] arguments)
        {
            ...
            foreach (var param in paramsStrings)
            {
                if (param.StartsWith("-"))
                {
                    ...
                    {
                        ConsoleLog.LogWarrning("参数中有无效值");
                        return;
                    }
                }//end if
            }
            ...
            if (type == "card")
            {
                ConsoleLog.LogWarrning("批量添加卡牌功能施工中...");
            }
            else
            {
                ConsoleLog.LogWarrning("参数不正确");
        ...
        private void generateRandomEquip(Dictionary<string, int> args)
        {
            ...
            ConsoleLog.LogWarrning("筛选条件:suitid = " + suitid + " eqtype = "+ eqtype + " ex = "+ ex);
        ...
        private List<int> generateRandomEquipIDRange(int suitid, int eqtype)
        {
            ...
            {
                ConsoleLog.LogWarrning("现有装备列表中不存在给定筛选条件的装备\n将从所有装备中随机生成");
                return equipment;
            }
        }
        ...
        private void generateRandomItem(Dictionary<string, int> args)
        {
            ...
            ConsoleLog.LogWarrning("筛选条件:count = " + count + " useType = " + useType);
